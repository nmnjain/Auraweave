...
import { Moon, ShieldAlert, LogOut } from 'lucide-react'; 
...
const MarketplaceNavbar: React.FC<MarketplaceNavbarProps> = ({
  ...
  onSwitchNetwork,
  onDisconnectWallet,
  onRequestTestTokens,  // Destructure
  isRequestingTokens,
      ...
      <div className={`${innerBoxBase} ${innerBoxBg} ${innerBoxJustify}`}>
        ...
        {isConnected && account && (
          <div className="flex items-center space-x-1.5 sm:space-x-2 md:space-x-3">
            ...
            {onRequestTestTokens && ( // Check if the function is provided
                <button
                    onClick={onRequestTestTokens}
                    disabled={isRequestingTokens}
                    className="bg-green-500 hover:bg-green-600 text-white font-semibold py-1 px-2 sm:py-1.5 sm:px-3 rounded-full text-xs transition-colors disabled:opacity-60 disabled:cursor-wait"
                    title="Get 100 test MUSDC for purchases"
                >
                    {isRequestingTokens ? (
                        <svg className="animate-spin h-4 w-4 inline" viewBox="0 0 24 24"> {/* Spinner */}
